...
from utils import convert_to_angle
...
def create_quantum_model(METHOD, num_qubit):
...
def main():
    ...
    #x_test = np.concatenate([x_test, x_train[-2000:]], axis=0)
    #y_test = np.concatenate([y_test, y_train[-2000:]], axis=0)
    ...
    # Use hinge loss
    y_train_hine = 2.0*y_train_nocon-1.0
    ...
    # Use partial samples (default all)
    ...
    import pickle
    with open(f"results/{METHOD}_{NUM_EXAMPLES}_{EPOCHS}_{LAYER}_{seed}_history.pk", 'wb') as f:
